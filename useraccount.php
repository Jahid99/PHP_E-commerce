<?php include 'inc/header.php'; ?>
 ...
 $result = $ui->showUserInfo($username);

 $row = mysqli_fetch_assoc($result);

 $res = $login->showLogin($username);
          ...
          if(isset($_POST['submit'])){
              ...
              // if everything is ok, try to upload file
                  ...
                  if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                      ...
                      $getthenameresult = $ui->showUserInfo($username);

                      $nameoftheuser = mysqli_fetch_assoc($getthenameresult);

                      $thisisthename = $nameoftheuser['firstname'] . ' ' . $nameoftheuser['lastname'];

                      $ui->updateUserInfo($fname,$lname,$address,$contactno,$email,$paypal,$img,$username);
